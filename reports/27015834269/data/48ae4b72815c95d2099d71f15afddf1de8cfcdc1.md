...
- Name: My Loans/inactive.spec.js >> My Loans - Inactive >> Search narrows results and pipeline sections remain visible
- Location: tests/My Loans/inactive.spec.js:60:9
...
```
Error: locator.clear: Test timeout of 180000ms exceeded.
Call log:
...
> 267 |             await this.searchInput.clear();
  ...
  350 |         // find the single "Open" button inside it, then use Playwright's .click()
  351 |         // which fires a full trusted pointer-event sequence (pointerdown, mousedown,
  352 |         // pointerup, mouseup, click).  MUI's onMouseDown handler requires
  353 |         // isTrusted:true — dispatchEvent() always produces isTrusted:false and is
  354 |         // therefore silently ignored by the MUI popup-indicator.
  355 |         const openBtn = dropdown.getByRole('button', { name: /open/i });
  356 |         await openBtn.waitFor({ state: 'visible', timeout: 10000 });
  357 |         await openBtn.click();
  358 | 
  359 |         // Step 2 — confirm the listbox is open and wait for options to populate.
  360 |         //
  361 |         // The listbox container can become visible before React renders its
  362 |         // children, so we wait for at least one [role="option"] before scanning.
  363 |         const listbox = this.page.getByRole('listbox');
  364 |         await expect(listbox).toBeVisible({ timeout: 10000 });
  365 |         await this.page.locator('[role="option"]').first()
  366 |             .waitFor({ state: 'visible', timeout: 5000 })
  367 |             .catch(() => { }); // proceed even if no options appear (empty list)
```